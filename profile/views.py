...
class ProfileView(DetailView):
    ...
    def get_object(self, queryset=None):
        self.profile = self.model.objects.select_related('profile').prefetch_related(
            "posts").prefetch_related("network").get(user_name=self.kwargs.get(self.slug_url_kwarg))
        return self.profile
    ...
    def get(self, request, *args, **kwargs):
        ...
        if title:
            context['page_obj'] = Paginator(Post.objects.all().filter(
                title__icontains=title, author=self.object, is_activate__exact=True), 6).get_page(page)
            print("resultado filtro !!!")
            print(context['page_obj'])
        else:
            context['page_obj'] = Paginator(Post.objects.all().filter(
                author=self.object, is_activate__exact=True), 6).get_page(page)
...
class EditPhotoProfile(UpdateView):
    ...
    def get_success_url(self):
        messages.success(
            self.request, 'Imagem de profile atualizada com sucesso!!!')
    ...
    def get_queryset(self):
        ...
        if user_name:
            profile_list = Profile.objects.filter(
                user__user_name__icontains=user_name)
            return profile_list
        if is_active:
